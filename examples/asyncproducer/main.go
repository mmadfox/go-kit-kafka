...
func main() {
	...
	asyncProducer := adaptersarama.NewAsyncProducer(producer)
	asyncProducer.OnError(
		func(err *sarama.ProducerError) {
			fmt.Println(err)
		})

	publisher := kafka.NewPublisher(asyncProducer)
	...
	for i := 0; i < 10; i++ {
		...
		go func(id int) {
			defer wg.Done()
			for j := 0; j < 50; j++ {
				msg := &kafka.Message{Topic: kafka.Topic(topic)}
